    ...
    <Card sx={styles.card}>
      ...
      <CardContent sx={styles.cardContent}>
        <Stack direction="column">
          ...
          <Rating value={rating} readOnly precision={0.5}></Rating>
        </Stack>
      </CardContent>
...
export default function MyBookShelfContent() {
  ...
  const fetchData = async () => {
    getLikedBooksByUsername(user.username)
      .then(async (likedBooks) => {
        const promises = likedBooks.map(async (likedBook) => {
          let avgRating = await getReviewsByBookID(likedBook._id)
            .then((result) => {
